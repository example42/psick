...
#
define example43::site (
    $mysql_user="root",
    $mysql_host="localhost",
    $mysql_database="",
    $mysql_password="",
    $backup_origin="",
    $vhost_template="example43/apache/default.erb",
    $vhost_priority="20",
    $vhost_alias="" ) {
    ...
    puppi::project::builder { "${name}":
        ...
        report_email     => "root@example43.com",
        enable           => "true",
    }
  ...
  if ($backup_origin == "yes") {
    ...
    file { "/etc/cron.daily/backup_${name}":
        ...
        owner   => "root",
        group   => "root",
        content => template('example43/apache/backup.erb'),
        tag    => "backup_origin",
    }
    ...
    if ($mysql_database != "") {
        file { "/etc/cron.daily/backupsql_${name}":
            ...
            owner   => "root",
            group   => "root",
            content => template('example43/apache/backupsql.erb'),
            tag    => "backup_origin",
        }
    }
