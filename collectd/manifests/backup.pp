# Class: collectd::backup
#
# Backups collectd data and logs using Example42 backup meta module (to be adapted to custom backup solutions)
...
# $collectd_backup_data (true|false) : Set if you want to backup collectd's data. Default: As defined in $backup_data
# $collectd_backup_log (true|false) : Set if you want to backup collectd's logs. Default: As defined in $backup_log
# $collectd_backup_frequency (hourly|daily|weekly|monthly) : Set the frequency of your collectd backups. Default: daily.
# $collectd_backup_target : Define how to reach (Ip, fqdn...) this host to backup collectd from an external server. Default: As defined in $backup_target
# 
...
class collectd::backup {
    ...
    backup { "collectd_data": 
        ...
        enabled   => "${collectd::params::backup_data_enable}",
    ...
    backup { "collectd_logs": 
        ...
        enabled   => "${collectd::params::backup_log_enable}",
        target    => "${collectd::params::backup_target_real}",
    }
    ...
    if $my_project { 
        case $my_project_onmodule {
            yes,true: { include "${my_project}::collectd::backup" }
            default: { include "collectd::backup::${my_project}" }
        }
    }
