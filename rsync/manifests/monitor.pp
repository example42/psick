# Class: rsync::monitor
#
# Monitors rsync process/ports/service using Example42 monitor meta module (to be adapted to custom monitor solutions)
...
# $rsync_monitor_port (true|false) : Set if you want to monitor rsync's port(s). If any. Default: As defined in $monitor_port
# $rsync_monitor_url (true|false) : Set if you want to monitor rsync's url(s). If any. Default: As defined in $monitor_url
# $rsync_monitor_process (true|false) : Set if you want to monitor rsync's process. If any. Default: As defined in $monitor_process
# $rsync_monitor_plugin (true|false) : Set if you want to monitor rsync using specific monitor tool's plugin  i   s. If any. Default: As defined in $monitor_plugin
# $rsync_monitor_target : Define how to reach (Ip, fqdn...) the host to monitor rsync from an external server. Default: As defined in $monitor_target
# $rsync_monitor_url : Define the baseurl (http://$fqdn/...) to use for eventual rsync URL checks. Default: As defined in $monitor_url
# 
...
class rsync::monitor {
    ...
    monitor::port { "rsync_${rsync::params::protocol}_${rsync::params::port}": 
        ...
        enable   => "${rsync::params::monitor_port_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::url { "rsync_url":
        ...
        enable   => "${rsync::params::monitor_url_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::process { "rsync_process":
        ...
        enable   => "${rsync::params::monitor_process_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::plugin { "rsync_plugin":
        plugin   => "rsync",
        enable   => "${rsync::params::monitor_plugin_enable}",
        tool     => "${monitor_tool}",
    }

    # Include project specific monitor class if $my_project is set
    if $my_project { include "rsync::${my_project}::monitor" }
