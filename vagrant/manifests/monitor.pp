# Class: vagrant::monitor
#
# Monitors vagrant process/ports/service using Example42 monitor meta module (to be adapted to custom monitor solutions)
...
# $vagrant_monitor_port (true|false) : Set if you want to monitor vagrant's port(s). If any. Default: As defined in $monitor_port
# $vagrant_monitor_url (true|false) : Set if you want to monitor vagrant's url(s). If any. Default: As defined in $monitor_url
# $vagrant_monitor_process (true|false) : Set if you want to monitor vagrant's process. If any. Default: As defined in $monitor_process
# $vagrant_monitor_plugin (true|false) : Set if you want to monitor vagrant using specific monitor tool's plugin  i   s. If any. Default: As defined in $monitor_plugin
# $vagrant_monitor_target : Define how to reach (Ip, fqdn...) the host to monitor vagrant from an external server. Default: As defined in $monitor_target
# $vagrant_monitor_url : Define the baseurl (http://$fqdn/...) to use for eventual vagrant URL checks. Default: As defined in $monitor_url
# 
...
class vagrant::monitor {
    ...
    monitor::port { "vagrant_${vagrant::params::protocol}_${vagrant::params::port}": 
        ...
        enable   => "${vagrant::params::monitor_port_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::url { "vagrant_url":
        ...
        enable   => "${vagrant::params::monitor_url_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::process { "vagrant_process":
        ...
        enable   => "${vagrant::params::monitor_process_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::plugin { "vagrant_plugin":
        plugin   => "vagrant",
        enable   => "${vagrant::params::monitor_plugin_enable}",
        tool     => "${monitor_tool}",
    }

    # Include project specific monitor class if $my_project is set
    if $my_project { include "vagrant::${my_project}::monitor" }
