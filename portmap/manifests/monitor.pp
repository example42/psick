# Class: portmap::monitor
#
# Monitors portmap process/ports/service using Example42 monitor meta module (to be adapted to custom monitor solutions)
...
# $portmap_monitor_port (true|false) : Set if you want to monitor portmap's port(s). If any. Default: As defined in $monitor_port
# $portmap_monitor_url (true|false) : Set if you want to monitor portmap's url(s). If any. Default: As defined in $monitor_url
# $portmap_monitor_process (true|false) : Set if you want to monitor portmap's process. If any. Default: As defined in $monitor_process
# $portmap_monitor_plugin (true|false) : Set if you want to monitor portmap using specific monitor tool's plugin  i   s. If any. Default: As defined in $monitor_plugin
# $portmap_monitor_target : Define how to reach (Ip, fqdn...) the host to monitor portmap from an external server. Default: As defined in $monitor_target
# $portmap_monitor_url : Define the baseurl (http://$fqdn/...) to use for eventual portmap URL checks. Default: As defined in $monitor_url
# 
...
class portmap::monitor {
    ...
    monitor::port { "portmap_${portmap::params::protocol}_${portmap::params::port}": 
        ...
        enable   => "${portmap::params::monitor_port_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::url { "portmap_url":
        ...
        enable   => "${portmap::params::monitor_url_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::process { "portmap_process":
        ...
        enable   => "${portmap::params::monitor_process_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::plugin { "portmap_plugin":
        plugin   => "portmap",
        enable   => "${portmap::params::monitor_plugin_enable}",
        tool     => "${monitor_tool}",
    }

    # Include project specific monitor class if $my_project is set
    if $my_project { include "portmap::${my_project}::monitor" }
