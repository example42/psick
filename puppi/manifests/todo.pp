...
# - Run of any kind of setup/configuration/initialization command that can't be automated
# It can also be used as a quick reminder on things done by hand and not Puppettized 
# for lack of time or skill. In this way it serves are a quick (Puppet managed) reminder
# on things to do in later steps.
# 
# Use the command puppi todo to show the todo presnet in your node.
# The exit status can be:
# 0 - OK - The task to do has been accomplished because the command specified as check_command
#          returns true (exit status 0)
# 1- WARNING - User hasn't specified a check_command to verify if the todo as been accomplished
# 2- ERROR - The task to do has not been accomplished becuase the commadn specified as check_command
#            returns an error (exit status different from 0)
#
# Usage:
