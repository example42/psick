...
describe 'profile::rabbitmq::tp' do
  on_supported_os(facterversion: '2.4').select { |k, _v| k == 'redhat-7-x86_64' || k == 'ubuntu-16.04-x86_64' }.each do |os, os_facts|
    context "on #{os}" do
      ...
      describe 'with auto_prereq => false' do
        ...
        it { is_expected.to contain_tp__install('rabbitmq').with(default_params.merge('auto_repo' => false, 'auto_prerequisites' => false)) }
      end
      ...
      describe 'with custom conf_hash' do
        ...
        it { is_expected.to contain_tp__conf('rabbitmq::main').with('ensure' => 'present', 'template' => 'profile/spec/sample.erb', 'options_hash' => sample_options_hash) }
        it { is_expected.to contain_tp__conf('rabbitmq::other').with('ensure' => 'present', 'source' => 'puppet:///profile/spec/sample') }
      end
    end
  end
end
