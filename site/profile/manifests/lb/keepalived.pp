# starting point for the keepalived-profile 
class profile::lb::keepalived (
  Enum['present','absent'] $ensure                     = 'present',

  Variant[String[1],Undef] $config_dir_source          = undef,
  String                   $config_file_template       = 'profile/lb/keepalived/keepalived.conf.erb',
) {
  ...
  $options_user=hiera_hash('profile::lb::keepalived::options', {} )
  ...
  # example: profile::lb::keepalived::config_dir_source: "puppet:///modules/profile/lb/keepalived/services/"
  ...
  $virtualservers.each | String $vs | {
    $vs_split=split($vs,'-')
    $app_role=$vs_split[1]
    $server_role=getvar("::profile::settings::${app_role}_server")
