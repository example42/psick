...
#                up with hiera_hash('profile::logs::rsyslog::options', {} )
#
class profile::logs::rsyslog (
  ...
  if $::os['family'] == 'RedHat' and $::os['release']['major'] == '7' {
    ::tp::conf { 'rsyslog::20-default.conf':
      ...
      template     => 'profile/logs/rsyslog/20-default.erb',
      options_hash => $options,
      base_dir     => 'conf',
    }
    ::tp::conf { 'rsyslog::50-default.conf':
      ensure       => $ensure,
      template     => 'profile/logs/rsyslog/50-default.erb',
      options_hash => $options,
      base_dir     => 'conf',
    }
  }
  ...
  if $server_ip != [] {
    ::tp::conf { 'rsyslog::60-syslogserver.conf':
      ...
      template     => 'profile/logs/rsyslog/60-syslogserver.erb',
      base_dir     => 'conf',
