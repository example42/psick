...
#                 up via hiera_hash('profile::hardening::pam::options', {} )
#
...
class profile::hardening::pam (
  ...
  $options_user=hiera_hash('profile::hardening::pam::options', {} )
  $options_default = {
    umask                    => '027',
    password_max_age         => 60,
    password_min_age         => 7,
  ...
  $real_system_auth_template = $system_auth_template ? {
    ''     => "profile/hardening/pam/system-auth_${::os['family']}${::os['release']['major']}",
    default => $system_auth_template,
  }
  $real_password_auth_template = $password_auth_template ? {
    ''     => "profile/hardening/pam/password-auth_${::os['family']}${::os['release']['major']}",
