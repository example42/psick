...
class profile::mongo::mms (
  ...
  String                 $monitoring_agent_template = 'profile/mongo/mms/monitoring-agent.config.erb',

  Boolean                $backup_agent_install      = true,
  String                 $backup_agent_template     = 'profile/mongo/mms/backup-agent.config.erb',

  Boolean                $automation_agent_install  = true,
  String                 $automation_agent_template = 'profile/mongo/mms/automation-agent.config.erb',
  ...
  $options=hiera_hash('profile::mongo::mms::options', { })
  $options_default = {
    'mmsApiKey'      => $apikey,
    'mmsBaseUrl'     => $baseurl,
    'maxLogFiles'    => '10',
  ...
  if $monitoring_agent_install {
    $monitoring_agent_user_settings=hiera_hash('profile::mongo::mms::monitoring_agent_settings', { })
  ...
  if $backup_agent_install {
    $backup_agent_user_settings=hiera_hash('profile::mongo::mms::backup_agent_settings', { })
  ...
  if $automation_agent_install {
    $automation_agent_user_settings=hiera_hash('profile::mongo::mms::automation_agent_settings', { })
  ...
  # master setup
  if $master_install {
    $master_user_settings=hiera_hash('profile::mongo::mms::master_settings', { })
