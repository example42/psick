# Class: exim::monitor
#
# Monitors exim process/ports/service using Example42 monitor meta module (to be adapted to custom monitor solutions)
...
# $exim_monitor_port (true|false) : Set if you want to monitor exim's port(s). If any. Default: As defined in $monitor_port
# $exim_monitor_url (true|false) : Set if you want to monitor exim's url(s). If any. Default: As defined in $monitor_url
# $exim_monitor_process (true|false) : Set if you want to monitor exim's process. If any. Default: As defined in $monitor_process
# $exim_monitor_plugin (true|false) : Set if you want to monitor exim using specific monitor tool's plugin  i   s. If any. Default: As defined in $monitor_plugin
# $exim_monitor_target : Define how to reach (Ip, fqdn...) the host to monitor exim from an external server. Default: As defined in $monitor_target
# $exim_monitor_url : Define the baseurl (http://$fqdn/...) to use for eventual exim URL checks. Default: As defined in $monitor_url
# 
...
class exim::monitor {
    ...
    monitor::port { "exim_${exim::params::protocol}_${exim::params::port}": 
        ...
        enable   => "${exim::params::monitor_port_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::url { "exim_url":
        ...
        enable   => "${exim::params::monitor_url_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::process { "exim_process":
        ...
        enable   => "${exim::params::monitor_process_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::plugin { "exim_plugin":
        plugin   => "exim",
        enable   => "${exim::params::monitor_plugin_enable}",
        tool     => "${monitor_tool}",
    }

    # Include project specific monitor class if $my_project is set
    if $my_project { include "exim::${my_project}::monitor" }
